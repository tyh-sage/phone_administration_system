            ...
            try {
               ...
               // ps.setString(4, txtemail);//将第四个?设置为参数txtemail
                ...
                while (rs.next()) {//本身就是遍历？
                	...
                	nNode.add(mphonenumber, mname, mdepartment, mtemail);//放入链表
                	nNode.quicksort2(nNode, nNode.getend(nNode));
                	
                    System.out.println("成功从数据库的worker表中获取数据：");
                    //System.out.println(mphonenumber + "\t" + mname + "\t"+mdepartment+ "\t"+mtemail);// “\t”为“转义字符”,代表的是一个tab，也就是8个空格。
                    flag="yes";
                }
//              while(nNode.next!=null) {System.out.println(nNode.name);nNode=nNode.next;}
                request.getSession().removeAttribute("pNode");
                request.getSession().setAttribute("pNode", nNode);
                request.removeAttribute("pNode");
                request.setAttribute("pNode", nNode);

                if(flag.equals("yes")) {
               	 	request.getRequestDispatcher("/phonenumber.jsp").forward(request, response);
//                	response.sendRedirect("sort.jsp");//实现重定向
                }
                else {
