...
<%
     ...
     node nnode = (node) request.getAttribute("nNode");
     String pflag =  request.getParameter("flag");
     String nflag =  request.getParameter("flag");
 %>
 <!-- Java传过来的flag，代表是否排好顺序-->
 <c:choose><c:when test="${pflag==yes}"><div class="top">
 <%
try{
...
try{
	while (nnode.next!=null) 
    {
     //将查询结果按表格形式输出 
	    out.print("<tr>");
	 	out.print("<td>"+nnode.phonenumber+"</td>");
	 	out.print("<td>"+nnode.name+"</td>");
	 	out.print("<td>"+nnode.department+"</td>");
	 	out.print("<td>"+nnode.email+"</td>");
	    out.print("<tr>");
	    nnode=nnode.next;
	}  	
...
%></table></c:when><c:otherwise></c:otherwise></c:choose>
